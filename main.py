...
from PySide6.QtCore import QSize, Qt, QRunnable, Slot, QThreadPool
...
class Gui(QMainWindow):
    ...
    #     for line in data:
    #         wd = WeatherData(6, True, line.get("url"), line.get("num"), line.get("town"))
    #         wd.run()

    def save_settings(self):
        settings.set_days(self.cmb_days.currentText())
        settings.set_nights(self.cmb_day_or_night.currentIndex())
        settings.set_target_folder(self.destination_dir.as_posix())
        settings.write_settings()

    def load_settings(self):
        self.cmb_days.setCurrentIndex(settings.get_days() - 1)
    ...
    def forecast_thread(self):
        ...
        forecast_thread = self.get_forecast(data)
    ...
    def get_forecast(self, data):
        ...
        for date_data in forecast.items():
            DiskIO.write_csv(date_data, (Path.cwd() / "tmp").as_posix())
    ...
    def setup_widgets(self):
        ...
        self.cmb_days.addItems(list(map(lambda x: str(x), range(1, 7))))
        ...
        btn_load_data.clicked.connect(self.forecast_thread)
        ...
        btn_address.setMinimumWidth(Sizes.widget_min_width)
